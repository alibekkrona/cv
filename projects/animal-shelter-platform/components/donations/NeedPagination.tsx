...
export function NeedPagination({
  currentPage,
  pageSize,
  searchParams,
  total,
  ...
  return (
    <nav className="mt-8 flex flex-col gap-4 border-t border-shelter-ink/10 pt-5 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <PageLink disabled={currentPage <= 1} href={buildPageHref(searchParams, currentPage - 1)} label="Назад" />
        {pageItems.map((item, index) => item === "ellipsis" ? (
          ...
          <PageLink key={item} active={item === currentPage} href={buildPageHref(searchParams, item)} label={String(item)} />
        ))}
        <PageLink disabled={currentPage >= totalPages} href={buildPageHref(searchParams, currentPage + 1)} label="Вперёд" />
      </div>
    </nav>
  );
}
